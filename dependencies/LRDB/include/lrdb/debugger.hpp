...
class debugger;
bool default_breakpoint_path_handler(debugger& debugger, std::string breakpoint_path, std::string executing_path) {
  return false; // fall back to default logic
}
...
class debugger {
 public:
  ...
  // addition by DieselLuaDebugger
  typedef std::function<bool(debugger& debugger, const std::string& breakpoint_path, const std::string& executing_path)> breakpoint_path_handler_type;

  debugger() : state_(0), pause_(true), step_type_(STEP_ENTRY), breakpoint_path_handler_(default_breakpoint_path_handler) {}
  debugger(lua_State* L) : state_(0), pause_(true), step_type_(STEP_ENTRY), breakpoint_path_handler_(default_breakpoint_path_handler) {
    reset(L);
  }
  ~debugger() { reset(); }
  ...
  void add_breakpoint(const std::string& file, int line,
    ...
    std::cout << "Adding breakpoint to " << file << " at line " << line << std::endl;
  ...
  /// @brief set breakpoint_path_handler. allows for additional logic to be set for what qualifies as an equivalent breakpoint file path
  void set_breakpoint_path_handler(breakpoint_path_handler_type handler) {
    breakpoint_path_handler_ = handler;
  }
  ...
  breakpoint_info* search_breakpoints(debug_info& debuginfo) {
    ...
    for (line_breakpoint_type::iterator it = line_breakpoints_.begin();
      ...
      if (currentline == it->line) {
        ...
        if (is_file_path_match(it->file.c_str(), source) || this->breakpoint_path_handler_(*this, it->file, source)) {
          return &(*it);
        }
      }
    }
...
public:
  ...
  breakpoint_path_handler_type breakpoint_path_handler_;
};
}  // namespace lrdb
